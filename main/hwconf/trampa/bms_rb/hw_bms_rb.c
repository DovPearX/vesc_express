...
static esp_err_t i2c_tx_rx(uint8_t addr,
	...
	if (read_buffer != NULL) {
		...
		res = i2c_master_bus_add_device(i2c_bus, &config, &device); if (res == ESP_OK) { res = i2c_master_transmit_receive(device, write_buffer, write_size, read_buffer, read_size, 500); i2c_master_bus_rm_device(device); }
	} else {
		i2c_device_config_t config = {.dev_addr_length = I2C_ADDR_BIT_LEN_7, .device_address = addr, .scl_speed_hz = 100000}; i2c_master_dev_handle_t device = NULL;
		res = i2c_master_bus_add_device(i2c_bus, &config, &device); if (res == ESP_OK) { res = i2c_master_transmit(device, write_buffer, write_size, 500); i2c_master_bus_rm_device(device); }
	}
...
static lbm_value ext_bms_init(lbm_value *args, lbm_uint argn) {
	...
	i2c_master_bus_config_t config = {.i2c_port = I2C_NUM_0, .sda_io_num = PIN_SDA, .scl_io_num = PIN_SCL, .glitch_ignore_cnt = 7, .flags.enable_internal_pullup = true};
...
static lbm_value ext_set_btn_wakeup_state(lbm_value *args, lbm_uint argn) {
	...
	case 0:
		esp_sleep_enable_ext1_wakeup_io(1ULL << PIN_ENABLE, ESP_EXT1_WAKEUP_ANY_HIGH);
		break;

	case 1:
		esp_sleep_enable_ext1_wakeup_io(1ULL << PIN_ENABLE, ESP_EXT1_WAKEUP_ANY_LOW);
		break;

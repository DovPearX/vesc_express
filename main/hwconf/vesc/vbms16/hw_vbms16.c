...
static esp_err_t i2c_tx_rx(
	...
	esp_err_t res = i2c_master_bus_add_device(i2c_bus, &device_config, &device);
	if (res != ESP_OK) {
	...
	if (read_size > 0 && read_buffer != NULL) {
		if (write_size > 0 && write_buffer != NULL) {
			res = i2c_master_transmit_receive(device, write_buffer, write_size,
					read_buffer, read_size, 500);
		} else {
			res = i2c_master_receive(device, read_buffer, read_size, 500);
		}
	} else {
		res = i2c_master_transmit(device, write_buffer, write_size, 500);
	}
	...
	switch (lbm_dec_as_i32(args[0])) {
		case 0:
			esp_sleep_enable_ext1_wakeup_io(
				1ULL << PIN_ENABLE, ESP_EXT1_WAKEUP_ANY_LOW
			);
			break;

		case 1:
			esp_sleep_enable_ext1_wakeup_io(
				1ULL << PIN_ENABLE, ESP_EXT1_WAKEUP_ANY_HIGH
			);
			break;

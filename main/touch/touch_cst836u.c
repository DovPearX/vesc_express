...
static i2c_master_dev_handle_t cst836u_device = NULL;
...
esp_err_t touch_cst836u_init(i2c_master_bus_handle_t bus, uint16_t width, uint16_t height, uint32_t frequency, lispif_touch_driver_t *driver) {
	...
	i2c_device_config_t config = {
		...
		.device_address = TOUCH_CST836U_I2C_ADDR,
		.scl_speed_hz = frequency,
	};
	esp_err_t res = i2c_master_bus_add_device(bus, &config, &cst836u_device);
	if (res != ESP_OK) {
		return res;
	}
	cst836u_width = width;
...
esp_err_t touch_cst836u_read_data(void) {
	...
	esp_err_t res = i2c_master_transmit_receive(cst836u_device, &reg, 1, raw, sizeof(raw), 20);
	if (res != ESP_OK) {
		cst836u_has_point = false;
		return res;
	}

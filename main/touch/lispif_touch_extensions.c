...
static esp_err_t touch_init_i2c_bus(int sda, int scl, uint32_t freq) {
	i2c_master_bus_config_t bus_config = {.i2c_port = TOUCH_I2C_PORT, .sda_io_num = sda,
		.scl_io_num = scl, .glitch_ignore_cnt = 7, .flags.enable_internal_pullup = true};
	esp_err_t res = i2c_new_master_bus(&bus_config, &touch_i2c_bus);
...
static lbm_value ext_touch_load_cst836u(lbm_value *args, lbm_uint argn) {
	...
	lispif_touch_driver_t driver = {0};
	res = touch_cst836u_init(touch_i2c_bus, (uint16_t)width, (uint16_t)height, &driver);
	if (res != ESP_OK) {
		lbm_set_esp_error_reason(res);
		return ENC_SYM_EERROR;
	}
